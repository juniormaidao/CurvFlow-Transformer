...
from torch_geometric.data import Dataset # 1.7.2
...
class GraphormerPYGDataset(Dataset):
    def __init__(
        ...
        if train_idx is None and train_set is None:
            ...
            self.train_data = self.create_subset(train_set[0]+valid_set[0]+test_set[0], train_set[1]+valid_set[1]+test_set[1])
            self.valid_data = self.create_subset(valid_set[0], valid_set[1])
            self.test_data = self.create_subset(test_set[0], test_set[1])
            self.train_idx = None
            self.valid_idx = None
            self.test_idx = None
        else:
    ...
    def index_select(self, idx):
        ...
        dataset.dataset = self.dataset.index_select(idx.tolist())
    ...
    def create_subset(self, subset, As):
        dataset = copy.copy(self)

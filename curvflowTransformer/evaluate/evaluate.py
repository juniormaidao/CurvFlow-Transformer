...
def custom_roc_auc_score(true_labels, predictions):
    # 找到不包含NaN值的索引
    non_nan_indices = ~np.isnan(true_labels)

    # 使用不包含NaN值的标签和预测值
    cleaned_labels = true_labels[non_nan_indices]
    cleaned_predictions = predictions[non_nan_indices]

    # 计算AUC
    auc_score = roc_auc_score(cleaned_labels, cleaned_predictions)
...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    auc_score = custom_roc_auc_score(np.array(y_true), np.array(y_pred))
